...
parser.add_argument('--model', default='ViT-B_16', type=str, choices=['ViT-B_16', 'ViT-L_16', 'ViT-B_16_IN21K', 'ViT_AugReg-B_16', 'ViT_AugReg-L_16', 'resnet50', 'resnet101', 'mlpmixer_B16', 'mlpmixer_L16', 'DeiT-B', 'DeiT-S', 'Beit-B16_224', 'Beit-L16_224'])
parser.add_argument('--epoch_size_test', default=1, type=int)
...
parser.add_argument('--loss_function', default='ours', type=str, choices=['tent', 'cfa', 'pl', 'shot-im', 't3a', 'attent'])
...
def main(args):
  ...
  minibatch_size_source = int(args.minibatch_size_train / args.ga_steps_train)
  minibatch_size_target = args.minibatch_size_test
  print(minibatch_size_source)
  print(minibatch_size_target)
  
  if calc_statistics_flag:
  ...
  if tta_flag:
      ...
      for j in range(len(CORRUPTION_SEVERITY)):
        ...
        errorrate_list_before = []
        errorrate_list_adapt = []
        celoss_list_before = []
        celoss_list_adapt = []
        entropy_list_before = []
        entropy_list_adapt = []
        ...
        for i in range(len(CORRUPTION_TYPE)):    
          ...
          del model
          model = load_model_from_saved_file(args)
          error_rate = testdata_adapt_and_evaluation(args, model, target_loader, statistics, adapt_flag=False)
          errorrate_list_before.append(error_rate)
          ...
          del model
          model = load_model_from_saved_file(args)
          error_rate = testdata_adapt_and_evaluation(args, model, target_loader, statistics, adapt_flag=True)
          errorrate_list_adapt.append(error_rate)
          print("Top-1 Error Rate: " + str(error_rate))
        ...
        print(errorrate_list_before)
        print(errorrate_list_adapt)
